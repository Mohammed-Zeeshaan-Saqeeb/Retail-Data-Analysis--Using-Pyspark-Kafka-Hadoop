...
spark = SparkSession \
    .builder \
    .appName("retail_data_analysis_stream") \
    .getOrCreate()
...
# defining  is_order UDF to check order or not
def is_order(x):
    if x == "ORDER":
        return (1)
    else:
...
# defining  is_return function to check if it is return or not.
def is_return(x):
    if x == "RETURN":
        return (1)
    else:
...
# defining total_cost udf for total order cost
